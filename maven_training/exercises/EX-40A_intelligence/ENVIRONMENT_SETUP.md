...
Load `EX-40A_BCT_Intel_Training_Data/` package (from training data package):
...
Place all files in: `[Training Project]/EX-40A/source/`
...
```
COMMANDER'S PIR GUIDANCE — EX-40A TRAINING
...
1. Navigate to: `[Training Project]/EX-40A/source/sigint_reporting` dataset
...
Evaluators record task Go/No-Go on standard EX-40A Evaluation Form (available from training NCO). Record specific failure modes in the notes column for post-exercise AAR. Overall Go/No-Go, participant name, evaluator name, and date required for training record submission.
