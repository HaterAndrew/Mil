# POST-TEST — TM-50A: ADVANCED ORSA
## Maven Smart System (MSS) — USAREUR-AF

| | |
|---|---|
| **Course** | TM-50A: Advanced ORSA |
| **Form** | Post-Test |
| **Level** | TM-50A (Advanced Specialist) |
...
B. Key parameters (demand, availability, failure rates) are uncertain and their probability distributions are known, enabling the optimizer to account for this uncertainty in the solution
C. The feasible region is bounded by more than 10 constraints
...
B. The probability of failure for every combination of parent node states, enabling inference when some parent values are observed
C. The sensitivity of the failure probability to changes in the prior distributions
D. The marginal distribution of the failure node after integrating over all parent states
...
B. Quantifying the expected outcomes and risk distributions for each COA, characterizing campaign-level uncertainty, and presenting decision-relevant findings in operational terms
C. Running the campaign simulation and validating results against classified intelligence estimates
...
B. Apply reconciliation (top-down, bottom-up, or optimal combination) to ensure coherent forecasts at all levels
C. Present both levels' forecasts to the commander and let the G4 decide which to use
D. Use the higher of the two estimates to ensure no shortfall

**10. The TM-50A standard for uncertainty communication to a GO audience requires that every analytical finding include:**

A. A p-value demonstrating statistical significance
B. The full posterior distribution plot for each parameter
C. A plain-language characterization of uncertainty (e.g., scenario range, robustness score, or confidence bounds) alongside the point estimate or key finding
...
A. The individual models agree — ensemble confirms accuracy
B. The individual models disagree — ensemble reduces variance and avoids reliance on any single model assumption
...
B. Methodology, key assumptions, data sources, scenario parameters, analytical limitations, and a statement that results represent a specific scenario set — not a prediction
C. The classified intelligence estimate used as the initial scenario conditions
D. Commander's written approval of the analytical method before the wargame

**13. A "state space model" for time series forecasting is preferred over a classical ARIMA model when:**

A. The time series has fewer than 30 data points
B. The underlying system has explicit structural components (trend, seasonality, irregular shocks) that change over time and can be modeled using a state evolution equation
C. The analyst needs a simpler model for GO-level briefing
D. The time series has been differenced to achieve stationarity

**14. TM-50A analytical product governance requires peer review by:**

A. The MSS program office before delivery
B. At least one other TM-50A qualified ORSA analyst before the product is delivered to the commander
C. The unit's Staff Judge Advocate for legal and ethical review
D. The G3 for operational relevance validation
...
B. Documenting methodology, training junior ORSAs, maintaining model reproducibility, and institutionalizing analytical standards so capability does not degrade with personnel turnover
C. Purchasing commercial OR software licenses that persist beyond the current ORSA's assignment
...
**SA-4. You have been tasked to deliver a 5-minute analytical brief to a LTG on adversary sustainment capability. Your analysis has significant uncertainty. Apply the TM-50A GO/SES briefing standard: describe the structure of the brief, what you will state in the BLUF, how you will communicate uncertainty, and what backup slides you will have prepared.**
...
**SA-5. Describe the TM-50A approach to building persistent OR capability in a command. What are the four key components of a persistent capability program, and what is the most significant risk if this program is not maintained?**
...
2. B — Stochastic optimization is preferred when parameter uncertainty has known distributions.
...
6. B — CPT specifies probability of child node state for every combination of parent node states.
7. B — ORSA quantifies outcomes, characterizes risk distributions, and presents decision-relevant findings.
8. B — Present robustness scores with scenario context, note probability weights not included, defer decision.
9. B — Reconciliation (top-down, bottom-up, or optimal) ensures coherent forecasts at all levels.
10. C — Plain-language uncertainty characterization alongside findings is the TM-50A GO standard.
11. B — Ensemble is most valuable when individual models disagree, reducing reliance on any single model.
12. B — Documentation must include methodology, assumptions, data sources, scenario parameters, limitations, and scenario-vs-prediction disclaimer.
13. B — State space models handle explicit structural components that evolve over time.
14. B — Peer review by at least one other TM-50A qualified ORSA is required before commander delivery.
15. B — Persistent capability = documented methodology, trained junior ORSAs, reproducible models, institutionalized standards.
