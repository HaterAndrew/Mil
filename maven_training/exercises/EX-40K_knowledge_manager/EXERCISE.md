...
- [ ] Create a Workshop form with fields: date, unit, exercise name, category (dropdown — use the 6-category taxonomy from TM-40K), observation (text), recommendation (text), OPR (responsible office), priority (H/M/L)
...
|------|---------------|--------------------|
| Task 1 | Form validation fires on focus, not on submit | Ask participant to fill out the form with a missing required field and click Submit — if the form submits, it is No-Go |
