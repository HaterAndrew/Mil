# CONCEPTS GUIDE — SL 3 COMPANION — ADVANCED BUILDER · MAVEN SMART SYSTEM (MSS)

> **Forward:** SL 3 is the shift from building to designing. A Builder (SL 2) configures tools. An Advanced Builder (SL 3) designs systems — pipelines that others feed, Ontology models that others extend, Workshop applications that commanders rely on for operational decisions. This guide develops the mental models required before those systems are built.
> **Purpose:** Establishes conceptual foundations for multi-source pipeline architecture, Ontology design, Workshop application composition, AIP Logic configuration, and data governance. Read before beginning SL 3 task instruction.
> *HQ USAREUR-AF · v1.0 · 2026 · DISTRIB: USG only*

---

## SECTION 1 — WHAT SL 3 ADDS

**BLUF:** SL 2 teaches you to use tools. SL 3 teaches you to design with them. The distinction is not a matter of technical skill — it is a matter of accountability. At SL 3, your designs become the infrastructure others depend on.

### 1-1. The Shift from Builder to Advanced Builder

At SL 2, the primary question is: *How do I use this tool to get this output?*

At SL 3, the primary question is: *What should this system look like, and what will break if I get it wrong?*

| Dimension | SL 2 Builder | SL 3 Advanced Builder |
|---|---|---|
| **Unit of output** | A single pipeline, widget, or Object Type | A system of interconnected pipelines, Objects, and applications |
| **User of your output** | Self or a specific analyst | Any operator (SL 1) or builder (SL 2) in the formation |
...
| **AIP Logic** | Aware of existing AI workflows | Configures parameters, monitors outputs, escalates authoring to SL 4H |
...
An error at SL 2 is contained. An error at SL 3 is systemic. When a SL 3 Advanced Builder designs an Object Type that other builders extend, names a property incorrectly, or configures a pipeline join at the wrong grain — the consequences propagate downstream. Applications break. Dashboard widgets go blank. SITREP counts double. Commanders see incorrect readiness data.

V Corps incident (2025): A SL 3 builder renamed a property on the `SoldierReadiness` Object Type in production without auditing downstream consumers. Three Workshop applications owned by G1, G4, and Brigade S3 went blank within 30 minutes. Restoration took four hours. The downstream impact assessment required before the change would have taken five minutes.

**SL 3 discipline is not process for its own sake. It is operational risk management.**

### 1-3. What SL 3 Does Not Cover

SL 3 is a UI boundary. Every SL 3 task is accomplished through Foundry's graphical tools. Where that boundary is, and what lies beyond it, is as important as what is within it.

| Requirement | SL 3 Scope? | Escalation |
|---|---|---|
...
| Python / PySpark transform authoring | NO | SL 4L |
| TypeScript Functions on Objects (FOO) | NO | SL 4H or SL 4L |
| OSDK integration or custom application code | NO | SL 4L |
| Authoring new AIP Logic workflows from scratch | NO | SL 4H |
| Configuring new source connector types | NO | SL 4L |
| Writing code-based data quality checks | NO | SL 4L |

Rule: if a requirement involves writing, editing, or debugging code in any language — it is SL 4.

---

## SECTION 2 — PIPELINE ARCHITECTURE

**BLUF:** A pipeline at SL 3 level is not a data filter — it is an architectural decision. How you design grain, join logic, and refresh behavior determines whether downstream consumers get reliable data or a compounding series of surprises.

### 2-1. Transform Graph Thinking

At SL 2, pipelines are linear: one source → one or two transforms → one output. At SL 3, pipelines are directed acyclic graphs (DAGs): multiple sources, multiple intermediate datasets, multiple output shapes, all connected in dependency chains.
...
### 2-4. Grain Management at Scale — The Primary SL 3 Pipeline Failure Mode
...
Commanders are SL 1 Operators. They use applications you build to make decisions — not to explore data. Design with these principles:
...
**BLUF:** SL 3 Advanced Builders configure and monitor AIP Logic workflows. They do not author them. Understanding what AIP Logic is and when to escalate to SL 4H is as important as knowing how to configure parameters.
...
**From a SL 3 perspective, AIP Logic is a configured pipeline with an LLM in the middle.** The pipeline structure, the Ontology connections, and the output handling are all visible in the AIP Logic UI. The LLM prompt itself is a configuration artifact — not code — but authoring a new prompt from scratch for an operational use case is SL 4H work.

### 5-2. SL 3 AIP Logic Scope vs. SL 4H Escalation

| Task | SL 3 Scope | SL 4H Required |
|---|---|---|
...
| Author a new AIP Logic workflow for a new operational use case | NO | SL 4H |
| Modify prompt text or prompt logic for an existing workflow | NO — escalate | SL 4H unless change is trivial and approved |
| Integrate AIP Logic output into a new Action that writes back to the Ontology | NO | SL 4H + C2DAO governance |
| Debug a workflow producing unexpected outputs (hallucination, format errors) | Diagnose and report | SL 4H for remediation |

### 5-3. Parameter Configuration — What SL 3 Controls

When a SL 4H AI engineer hands off a configured AIP Logic workflow, SL 3 is responsible for:

| Parameter | What It Controls | SL 3 Guidance |
|---|---|---|
...
> **CAUTION:** Do not attempt to fix AIP Logic workflow errors by adjusting parameters you do not fully understand. A temperature change or data filter modification can alter workflow behavior in ways that are not visible until the workflow runs on real data. Report anomalies to SL 4H; do not troubleshoot by trial and error on a production workflow.

### 5-5. NLQ Setup and Monitoring

Natural Language Query (NLQ) allows users to query the Ontology in plain language. SL 3 configures the synonym tables and ontology-to-NLQ mappings that determine what queries the system understands.
...
**BLUF:** Governance at SL 3 is not a checklist item — it is the operational discipline that prevents your work from breaking the formation's data picture. C2DAO (Command and Control Data Authoritative Organization) is the authority. Branch workflow is the mechanism. Naming standards are the enforcement.
...
**The branch workflow is the mechanism that prevents development from corrupting production.** At SL 3, you execute this workflow — you do not just understand it exists.
...
## SECTION 7 — SL 3 VS. SL 4 DECISION BOUNDARY

**BLUF:** SL 3 owns the UI layer. SL 4 owns the code layer. When a requirement crosses the UI boundary, escalation is the correct action — not improvisation. Know the boundary before you encounter it in the field.

### 7-1. Full Decision Table

| Requirement | SL 3 Owns | SL 4 Track | Notes |
|---|---|---|---|
| Workshop multi-page application design and build | YES | — | Full SL 3 scope per Chapter 2 |
| Variable chains, conditional visibility, cascading filters | YES | — | Full SL 3 scope per Chapter 2 |
| Multi-source pipeline join with grain management | YES | — | Full SL 3 scope per Chapter 3 |
| Ontology Object Type and Link Type design | YES | — | Full SL 3 scope per Chapter 4 |
| Action schema design (no-code Actions) | YES | — | Full SL 3 scope per Chapter 5 |
| AIP Logic parameter configuration and monitoring | YES | — | Full SL 3 scope per Chapter 6 |
| Python / PySpark pipeline authoring | NO | SL 4L (SWE) | Refer when data volume or logic complexity exceeds Pipeline Builder |
| Incremental transform logic requiring @incremental decorator | NO | SL 4L (SWE) | SL 3 configures schedule; SWE authors the transform logic |
| TypeScript Functions on Objects (FOO) | NO | SL 4H (AI Eng) or SL 4L (SWE) | FOO requires TypeScript authoring |
| New AIP Logic workflow authoring | NO | SL 4H (AI Eng) | SL 3 configures existing workflows |
| Agent Studio workflow authoring | NO | SL 4H (AI Eng) | Above SL 3 scope |
| Statistical modeling, regression, Monte Carlo | NO | SL 4G (ORSA) | ORSA methodology required |
| ML model development and integration | NO | SL 4M (MLE) | Model development is code-layer |
| OSDK application development | NO | SL 4L (SWE) | Custom application code |
| Data product governance program management | SL 3 enforces standards | SL 4J (PM) for enterprise-level program coordination | PM track for cross-organizational governance programs |
| Data product documentation and taxonomy | SL 3 applies standards | SL 4K (KM) for organizational knowledge architecture | KM track for enterprise knowledge design |
| Source connector configuration (new connector types) | NO | SL 4L (SWE) | Existing connector configuration is SL 3 scope; new types are not |
| Custom data quality checks in code | NO | SL 4L (SWE) | UI-based quality checks are SL 3; code-based checks are SL 4L |

### 7-2. The Escalation Standard

Escalating is not failure. Escalating when a requirement exceeds your lane is the correct action. Attempting to build something that requires SL 4 capability at the SL 3 level produces:

- A fragile solution that breaks when the underlying data changes
- A solution that cannot be maintained by any SL 4 engineer (because it circumvents proper tool use)
- A C2DAO compliance violation if the workaround produces unauthorized data writes

**Document the requirement, identify the correct SL 4 track, and route it.** If you are uncertain which SL 4 track owns a requirement, refer to the escalation decision table in Section 9.

---

## SECTION 8 — ANTI-PATTERNS

**BLUF:** The following failures appear routinely in SL 3-level work across the formation. Recognize them before building. Most are easier to prevent than to fix after production deployment.
...
| 7 | **AIP Logic prompt modification without escalation** | Workflow output changes unexpectedly after "minor" parameter edits | SL 3 modifying prompt text that is SL 4H scope | SL 3 adjusts parameters only; any prompt text change routes to SL 4H |
...
**BLUF:** When a requirement exceeds SL 3 scope, route it to the correct SL 4 specialist. Do not route everything to SL 4L because "it needs code." Each specialist has a lane.

| Signal | Route to | Why |
|---|---|---|
| Requirement needs Python or PySpark transform authoring, OSDK integration, or custom connector configuration | **SL 4L — Software Engineer** | Code-layer pipeline and application development is SWE lane |
| Requirement needs a new AIP Logic workflow authored from scratch, prompt engineering, Agent Studio, or TypeScript Functions on Objects | **SL 4H — AI Engineer** | LLM workflow design and AI reasoning layer is AI Eng lane |
| Requirement involves statistical analysis: regression, forecasting, simulation, Monte Carlo, decision support modeling | **SL 4G — ORSA** | Quantitative analysis methodology is ORSA lane; tools alone do not make analysis valid |
| Requirement needs a machine learning model built, trained, validated, or integrated into the Ontology | **SL 4M — ML Engineer** | Model development and inference pipelines are MLE lane |
| Requirement involves cross-organizational coordination, data product governance program, stakeholder management across multiple units or functional areas | **SL 4J — Program Manager** | Enterprise governance coordination is PM lane |
| Requirement involves building or restructuring a knowledge taxonomy, documentation architecture, or data product catalog for the organization | **SL 4K — Knowledge Manager** | Organizational knowledge architecture is KM lane |
| Requirement appears to need multiple specialist tracks (e.g., ML model + Workshop application + governance program) | **SL 4J (PM) as coordinator first**, then PM routes to other specialists | Complex requirements need a PM to coordinate scope and delivery across lanes |

> **NOTE:** When escalating, document the requirement clearly before routing — do not hand off a vague verbal description. Write down: what the user needs, what data is involved, what you have already assessed as SL 3 capable vs. not, and what the deadline is. A clear handoff accelerates SL 4 response.

---

## SECTION 10 — PRE-BUILD CHECKLIST

**BLUF:** Complete this checklist before beginning any SL 3 design or build task. If any item cannot be confirmed, resolve it before building. Building on an unconfirmed foundation produces rework.
...
| 2 | Scope boundary assessed — confirmed whether the requirement is fully within SL 3 scope or requires SL 4 escalation for any component | |
...
Proceed to SL 3 task instruction when you can answer all six questions without consulting this guide.

**1. What is the difference between a SL 2 Builder and a SL 3 Advanced Builder in terms of accountability and downstream impact?**

A SL 2 builder's errors are typically self-contained. A SL 3 Advanced Builder's errors propagate to every downstream consumer of shared Ontology artifacts, pipelines, and applications. SL 3 accountability is formation-level.
...
**4. What does SL 3 own in AIP Logic, and what requires escalation to SL 4H?**

SL 3 owns parameter configuration (temperature, max tokens, data filter bindings, NLQ synonyms, run schedule) and monitoring. Authoring new workflows, modifying prompt text, authoring TypeScript Functions on Objects, and integrating AIP Logic output with Actions that write to the Ontology are SL 4H scope.
...
**6. An analyst asks you to build a statistical regression model predicting equipment failure probability based on maintenance history. Is this SL 3 scope? If not, where does it route?**

Not SL 3 scope. Statistical modeling requiring regression analysis routes to SL 4G (ORSA). If the requirement also involves building an ML pipeline to serve the model at inference time, that component routes to SL 4M (ML Engineer). SL 3's contribution is the Workshop application that displays ORSA or MLE outputs — not the analysis or model itself.
...
- **Escalation trigger:** If the requirement adds a predictive maintenance model, route the model to SL 4M; SL 3 builds the Workshop display layer only.

> **NOTE:** Similar dashboard patterns apply to other MSCs. 10th AAMDC requires Patriot/THAAD launcher readiness tracking with sensor-status overlays. SETAF-AF needs expeditionary vehicle readiness across dispersed Africa AOR locations with limited connectivity. The SL 3 design principles — grain management, incremental refresh, role-based access — are identical; the data sources and operational context differ.
...
- **Ontology fix:** Change `sitrep_status` from a free-text property to an enumerated property with controlled vocabulary: `{FMC, PMC, NMC, DEADLINE}`. This is a SL 3 Ontology design task.
- **Pipeline fix:** Add a normalization transform that maps incoming string values to the controlled vocabulary before Ontology ingestion. This is a SL 3 Pipeline Builder task if achievable with no-code transforms. If the normalization logic requires a Python lookup table, escalate the transform to SL 4L; SL 3 owns the Ontology property design regardless.
...
This guide establishes the mental models applied throughout SL 3. SL 3 task instruction assumes this conceptual foundation. It will not re-explain why branch workflow is mandatory, why grain management determines pipeline correctness, or why primary key stability is the most consequential Ontology design decision.

Proceed to TM-30, Chapter 1, when you can answer the six gating questions in Section 11 without referencing this guide.

When SL 3 qualification is complete, select a SL 4 specialist track based on assigned duties and billet. All specialist tracks (SL 4G through SL 4O) require SL 3 as a hard prerequisite.
...
|---|---|---|
| NATO Data Strategy for the Alliance (Feb 2025) | NATO | Alliance-wide data governance mandate — governs coalition data sharing requirements applicable to SL 3 ontology design |
| NATO Data Quality Framework for the Alliance (Aug 2025) | NATO | Quality governance and metrics — applicable to multi-national data products built at SL 3 level |

---

## CURRICULUM NOTES

**Prerequisites:** SL 1 (Maven User) and SL 2 (Builder) are REQUIRED before beginning SL 3 or this guide. The Data Literacy Technical Reference is also required reading.

**Next steps after SL 3:**
- WFF tracks (SL 4A–F): for Intelligence, Fires, Movement and Maneuver, Sustainment, Protection, and Mission Command operators. Prereq is SL 3 (required).
- Specialist tracks (SL 4G–O): ORSA, AI Engineer, ML Engineer, Program Manager, Knowledge Manager, Software Engineer, UX Designer, Platform Engineer. Prereq is SL 3 (required).

**Peer specialist cross-references:**
- **SL 4H (AI Engineer):** SL 3 configures AIP Logic workflows that SL 4H authors. The boundary is clear: SL 3 adjusts parameters; SL 4H changes prompts, workflow logic, and Ontology write-back integrations.
- **SL 4L (Software Engineer):** SL 3 builds no-code pipelines; SL 4L authors Python/PySpark transforms when Pipeline Builder cannot handle the logic. Know the boundary before routing work.
- **SL 4G (ORSA):** SL 3 builds the Workshop display layer for ORSA analysis outputs. The ORSA designs the methodology and owns the analytical validity; SL 3 makes it accessible to operators.
- **SL 4K (Knowledge Manager):** SL 3 applies naming and documentation standards that SL 4K establishes. When the naming standard is unclear or the data catalog structure needs redesign, SL 4K is the authority.
