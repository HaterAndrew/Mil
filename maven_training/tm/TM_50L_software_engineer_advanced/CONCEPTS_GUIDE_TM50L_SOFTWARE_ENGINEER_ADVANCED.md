# CONCEPTS GUIDE — TM-50L COMPANION
...
TM-50L is not about writing more complex code than TM-40F. It is about taking ownership of the platform's code health, architectural patterns, and engineering standards — and shaping how the entire team writes, reviews, tests, and deploys software.
...
At TM-50L, the measure of success is different. The advanced software engineer asks: does the code work, and does it work in a way that makes the platform stronger over time? Those are different questions. A transform can process data correctly while accumulating technical debt that will collapse under production load in six months. A Workshop application can function for its current users while being architected in a way that cannot be extended to serve the next mission requirement. Code that "works" is necessary but not sufficient at TM-50L.

The TM-50L engineer operates across three dimensions simultaneously:

| Dimension | TM-40F Focus | TM-50L Focus |
|---|---|---|
...
A team with one excellent engineer and eight mediocre engineers produces mediocre outcomes at scale. The excellent engineer can write excellent code for their own work, but they cannot review all code, fix all designs, or prevent all production incidents alone. The force multiplier for a TM-50L engineer is not their individual output — it is their ability to make every engineer on the team more effective.
...
TM-50L engineers understand not just how to write transforms but how the Foundry platform executes them: Spark execution, dataset partitioning, compute resource allocation, and performance reasoning at scale. The diagnostic skill — moving from symptoms to root cause — is the advanced competency.
...
The key architectural insight: Spark moves data across the network during shuffle operations. A join between two large datasets requires both datasets to be reorganized by join key across the cluster — this is expensive. Every wide operation is a potential performance bottleneck. The TM-50L engineer designs transforms to minimize shuffles: reduce data early, filter before joining, partition by the key that downstream operations will use.
...
**Partitioning design decisions at TM-50L level:**
...
Foundry allocates compute resources (CPU cores, memory) to transform builds. TM-50L engineers understand when and how to configure resource allocation — and, more importantly, when resource configuration is the wrong solution to a performance problem.
...
3. Is the transform reprocessing data it has already processed? Incremental processing with watermarks (TM-50L task territory) eliminates reprocessing of historical data.

### Symptoms-to-Root-Cause Reasoning

The following table maps common production symptoms to diagnostic hypotheses. TM-50L engineers use this framework before changing resource configuration or requesting platform support.
...
At TM-50L level, the SWE contributes to Ontology design decisions that affect the entire platform. The critical competency is evaluating a design proposal for scalability before it is implemented — not after it fails under production load.
...
The TM-40F engineer learns to implement Ontology definitions correctly. The TM-50L engineer learns to evaluate Ontology definitions for long-term scalability before they are merged.

### Object Type Inheritance Patterns

Foundry's Ontology does not support classical object inheritance in the object-oriented sense. There is no "extends" relationship between Object Types. However, TM-50L engineers can model inheritance-like relationships through two patterns, each with distinct tradeoffs:
...
Performance considerations for TM-50L engineers:
...
The TM-50L engineer's rule: if a computed property will be accessed by Workshop tiles that serve many users simultaneously, model the computation carefully. A property that is cheap to compute for one user becomes expensive when ten Workshop instances request it simultaneously for large Object Sets.
...
Before approving an Ontology design proposal, the TM-50L engineer should evaluate it against these questions:
...
At TM-50L level, OSDK applications go beyond displaying Object data. Advanced patterns include caching strategies, real-time updates, multi-Object-Type design, and authentication/authorization for applications serving multiple roles with different access levels.
...
The OSDK client maintains an in-memory cache of Objects it has loaded. Subsequent accesses to the same Objects return cached values without network round-trips. The TM-50L engineer understands the cache invalidation model: when does the cache update? What is the staleness window? For near-real-time applications, staleness of a few minutes may be unacceptable. For historical analysis applications, staleness of hours is irrelevant.
...
Security is a design constraint applied from the beginning of every system, not a feature added at the end. At TM-50L level, the engineer evaluates every component for its security model before writing the first line of code.

### The Security Design Mindset

TM-40F engineers learn to implement security controls: apply CBAC markings, use HTTPS, rotate credentials. TM-50L engineers design security in from the architecture phase: before choosing a data storage pattern, an integration approach, or an application architecture, ask — what is the security model of this design, and does it satisfy the requirement?

The questions that precede every design decision at TM-50L level:
...
The Ontology's CBAC model controls which users and groups can read, write, or execute Actions on specific Object Types. TM-50L engineers design Ontology access control schemas, not just implement them.
...
The TM-50L security review checklist for Actions:
...
### What a Security Review of a Code PR Looks Like at TM-50L

Security review is not a separate audit performed by a security team — it is integrated into the code review that TM-50L engineers conduct on every PR. The following checks are mandatory at TM-50L level:
...
The TM-50L engineer designs and maintains the automated pipeline that moves code from development to production. An effective CI/CD pipeline enforces governance requirements without creating bottlenecks that slow the team.
...
Foundry's promotion workflow moves a verified code branch from a development environment to a production environment. The TM-50L engineer designs this workflow to include checkpoints that enforce the team's deployment standards.
...
TM-50L engineers lead code reviews. Effective code review evaluates correctness, design, maintainability, security, and operational risk — and delivers feedback in a way that improves the reviewer without demoralizing them.
...
Every production change has a blast radius — the scope of impact if the change fails. TM-50L engineers make blast radius assessment explicit in code review.
...
**Principles for TM-50L code review feedback:**
...
At TM-50L level, documentation is an engineering artifact with the same lifecycle as code. Missing documentation is a defect. The operational cost becomes apparent when a system fails at 0300 and no one knows how to recover it.
...
The failure modes at TM-50L level are different from those at TM-40F. They are not beginner mistakes — they are predictable errors made by capable engineers who have access to powerful tools and face genuine operational pressures.

### Failure Mode 1 — Over-Engineering

The MSS platform provides sophisticated tools: incremental processing, Ontology subscriptions, complex OSDK patterns, multi-hop link traversals, and streaming integrations. TM-50L engineers have mastered these tools. The failure mode is applying them when simpler approaches would serve the mission better.
...
TM-50L engineers who underinvest in observability typically do so because observability instrumentation is not immediately functional: it does not make the system work better today. The investment pays off only when something goes wrong. Until it goes wrong, the investment looks like overhead.
...
Minimum observability requirements for TM-50L-level systems:
...
The TM-50L lead conducting the review identifies the following issues, none of which were caught in functional testing:
