...
EXEC is a 1-day instructor-led course that produces senior leaders who can:
...
**EXEC replaces SL 1 for O-5 and E-9+ personnel.** Senior leaders do not attend SL 1. EXEC is their entry point and terminal qualification — there is no progression to SL 2 or beyond. If a senior leader wants to build, they are welcome to enroll in SL 1 and proceed through the standard pipeline. EXEC does not grant SL 1 credit.
...
- It is not a substitute for SL 1 in the standard training pipeline — SL 1 remains the entry point for all non-SL personnel
- It does not qualify you to build, modify, or administer anything on the platform

## 1-4. Scope Boundaries with SL 1

SL 1 teaches personnel how to use MSS: log in, navigate, read dashboards, filter data, submit forms, export products, and handle classification. EXEC teaches none of that. There is no overlap.

EXEC assumes you have staff who do SL 1-level work. Your job is to understand what those staff produce, why it matters, and how to make it better through command emphasis and informed guidance.

| SL 1 Covers | EXEC Covers Instead |
|---|---|
...
The following doctrine and policy documents establish the authority, framework, and standards under which MSS operates. SL 1 introduces these references at the operator level. At EXEC, you need to know them at the level required to make resourcing and governance decisions.
...
- **AR 350-1, Army Training and Leader Development** — Governs training policy and leader development. The training pipeline described in Chapter 4 (SL 1 through SL 5) is structured under AR 350-1 standards.
- **FM 7-0, Training** — Unit training management procedures. Provides the context for how EXEC and the broader MSS training pipeline integrate with your formation's training plan.
...
|---|---|---|---|
| **Ingestion** | Brings data in from Army systems (GCSS-A, DCPDS, MEDPROS, unit feeds) | SL 3/SL 4 engineers | Stale feeds, broken connections, missing source systems |
| **Storage & Modeling** | Organizes raw data into structured, queryable formats | SL 3/SL 4 engineers | Poor naming, duplicate records, broken relationships between datasets |
| **Transformation** | Cleans, links, and enriches data for consumption | SL 2/SL 3 builders | Bad business logic, incorrect calculations, filters that hide real data |
| **Application** | Dashboards, forms, reports — what your staff sees | SL 2 builders | Misleading visualizations, missing filters, stale data displayed as current |
| **AI/Analysis** | AIP queries, pattern detection, automated alerts | SL 4H/M engineers | Outputs that look authoritative but reflect training data, not ground truth |
...
|---|---|---|---|
| **Readiness dashboard** | Equipment OR rates by subordinate unit, updated daily | SL 2 builder | CDR, XO, S3, S4 |
| **Personnel tracker** | Strength accounting across the formation | SL 2 builder | CSM, S1, CDR |
| **Operational report** | SITREP data aggregated from subordinate submissions | SL 2/SL 3 builder | CDR, higher HQ |
| **Logistics status** | Maintenance backlog, parts on order, deadline rates | SL 2 builder | S4, SPO, CDR |
| **WFF analysis** | Intel threat picture, fires target set, protection posture | SL 4A–F specialist | WFF staff, CDR |
| **Automated alert** | Threshold-based notification (e.g., readiness drops below 80%) | SL 3/SL 4 engineer | CDR, staff principal |
| **AI-generated summary** | AIP query response synthesizing multiple data sources | SL 4H/M engineer | Staff, CDR |
...
> **NOTE:** You are seeing these tools so you understand what your staff works with. You are not expected to use them yourself. If you want hands-on qualification, enroll in SL 1.
...
| **Single point of failure** | One SL 2 builder owns all the unit's dashboards; that person PCSes | No cross-training, no documentation, no succession plan | Treat builder capacity like any other critical MOS — plan for turnover, cross-train, document |
...
|---|---|---|---|---|
| **EXEC** | Senior Leader | 1 day | O-5 / E-9+ | Lead formations that use MSS; evaluate data products; guide data posture |
| **SL 1** | Maven User | 1 day | All other personnel | Use MSS as a consumer: navigate, filter, export, submit |
| **SL 2** | Builder | 3 days | Designated builders | Build Workshop apps, basic dashboards, simple pipelines (no code) |
| **SL 3** | Advanced Builder | 5 days | Advanced builders | Complex pipelines, governance, data modeling, enterprise ontology |
| **SL 4A–F** | WFF Tracks | 3 days each | WFF staff (S2, FSE, S4, etc.) | Apply MSS to their warfighting function |
| **SL 4G–O** | Specialist Tracks | 5 days each | Data professionals | ORSA, AI, ML, PM, KM, SWE, UX, PE specializations |
| **SL 5G–O** | Advanced Specialist | 5 days each | Senior specialists | Advanced application of specialist skills |

**Prereq chain:**
```
EXEC (senior leaders — terminal, no further progression)

SL 1 (all other personnel)
  └── SL 2
        └── SL 3
              ├── SL 4A–F (WFF tracks)
              └── SL 4G–O → SL 5G–O (specialist tracks)
```

> **NOTE:** EXEC does not feed into the TM pipeline. A senior leader who completes EXEC and later wants hands-on qualification enrolls in SL 1 and progresses normally.

## 4-2. Foundry Bootcamp (FBC)

The Foundry Bootcamp is a quarterly 5-day supervised build event outside the TM chain. A SL 2-qualified builder brings a command-approved operational project and builds it with SME support in the room.

FBC does not grant SL 3 credit or unlock SL 4. It exists because some builders learn best when solving a real problem under pressure. It is an excellent use of builder time when your formation has a specific project that needs to get built.

**What the commander needs to know about FBC:**
- Prereq: SL 2 Go + command-approved project
...
|---|---|
| **How many builders?** | Minimum viable: 2 SL 2 builders per BN, 4–6 per BDE. More is better. Plan for PCS turnover. |
| **When to train?** | SL 1 can run locally. SL 2+ requires coordination with C2DAO for instructor support and training environment access. Build into your training calendar, not your white space. |
...
- **SL 1 Go** — This person can use MSS. They can find data, read dashboards, submit information, and export products. They cannot build or modify anything.
- **SL 2 Go** — This person can build. They can create Workshop dashboards, set up basic data pipelines, and configure forms. They are your unit-level data product developers.
- **SL 3 Go** — This person can design. They can build complex pipelines, work with the enterprise ontology, and handle governance-level configuration. They are your formation's data architects.
- **SL 4 Go** — This person is a functional or technical specialist. They apply MSS to a specific warfighting function or technical domain at a professional level.
...
| Invest in their training pipeline | Expect them to figure it out | SL 2 → SL 3 → SL 4 is a deliberate progression. Skipping steps produces builders who can operate tools but cannot design solutions |
...
| **Training** | Who is in the pipeline? When is the next SL 2 class? Are we sending the right people? |
...
EXEC is terminal — there is no EXEC-20 or progression requirement. But the course is only useful if you apply what you learned. Here is a practical checklist:
...
- [ ] Review your builder manning: how many SL 2+ qualified personnel do you have, and is that enough?
- [ ] Schedule your next round of SL 1/SL 2 training for personnel who need qualification
...
| If You Want To... | Path |
|---|---|
| Get hands-on with the platform | Enroll in SL 1 and proceed through the standard pipeline |
| Send builders to advanced training | Ensure SL 2 Go → enroll in SL 3 → select SL 4 track |
| Get a project built quickly | Submit a FBC project for the next quarterly bootcamp (requires SL 2 builder + command-approved project) |
...
*EXEC Senior Leader Executive Course | Version 1.0 | March 2026*
