# CONCEPTS GUIDE — TM-50A COMPANION
...
This guide is not a task manual. It does not walk through platform procedures, code steps, or checklist sequences. Those are in TM-50A. This guide addresses the harder problem: how a senior ORSA practitioner thinks.
...
Read this guide once before beginning TM-50A. Return to specific sections when you encounter a problem in practice that matches the conceptual territory described here.
...
9. Advanced Failure Modes — What TM-50A ORSAs Get Wrong
...
**BLUF:** At TM-50A level, the ORSA is not just answering questions — they are shaping which questions get asked. This is the most important conceptual transition in the entire TM-50A qualification.
...
Analytical wargaming in the ORSA context is distinct from operational wargaming. Operational wargaming explores adversary courses of action, evaluates friendly COAs, and stress-tests plans against threat actions. Analytical wargaming, as applied by the TM-50A ORSA, uses scenario modeling to stress-test the analytical models and assumptions that underpin an operational plan.
...
**Compressed decision cycles.** Theater-level decisions in a crisis environment may have hours, not days, for analytical support. This is not a failure condition — it is the normal operating environment. The TM-50A ORSA must design products that can be updated rapidly under time pressure: modular analyses where the central estimate can be refreshed with new data without rebuilding the entire pipeline, and briefing formats that convey confidence levels in seconds rather than minutes.
...
**BLUF:** An analytical product that only one person can run is a liability, not an asset. At TM-50A level, building transferable pipelines is as important as building correct ones.
...
A TM-50A ORSA-designed pipeline should meet the following transferability standards:
...
## SECTION 9 — ADVANCED FAILURE MODES: WHAT TM-50A ORSAs GET WRONG
...
Before proceeding to TM-50A task procedures, confirm you can answer the following questions from memory. These reflect the conceptual content of this guide and the expected mental models for TM-50A-level practice.
...
7. What six elements constitute a transferable analytical pipeline under TM-50A standards?
...
## APPENDIX B — CROSS-REFERENCE TO TM-50A CHAPTERS

| Concepts Guide Section | Corresponding TM-50A Chapter |
|------------------------|------------------------------|
...
*CONCEPTS GUIDE — TM-50A COMPANION // ADVANCED ORSA // UNCLASSIFIED*
