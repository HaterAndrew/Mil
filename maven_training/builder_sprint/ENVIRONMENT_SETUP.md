# BUILDER SPRINT — ENVIRONMENT SETUP
## Coordinator Checklist — Pre-Sprint Configuration
...
| T-10 days | Sprint workspace created in MSS Training Environment | MSS Administrator |
...
The MSS Administrator creates a dedicated sprint workspace in the MSS Training Environment:

- **Naming convention:** `BSP_[YYYYQQ]_SPRINT` (e.g., `BSP_202602_SPRINT`)
...
> **NOTE:** If a participant's project requires production data, coordinate with C2DAO and the unit Data Steward separately. Production access during BSP is not automatic and requires explicit approval before T-10 days.

### 2. Participant Sub-Projects

Create a sub-project folder for each participant within the sprint workspace:
...
- A README template (from BSP_GUIDE.md Appendix B)
- The naming standards reference card (quick_reference/cheatsheet.md)
- A branch pre-configured for the participant's sprint work (branch name: `[PARTICIPANT_LASTNAME]/sprint`)
...
- [ ] Access their sub-project in the sprint workspace
...
3. Sprint schedule and standup format
...
*Builder Sprint (BSP) Environment Setup | Version 1.0 | March 2026*
