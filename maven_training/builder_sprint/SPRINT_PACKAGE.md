# BUILDER SPRINT — EVALUATOR AND COORDINATOR PACKAGE
## Maven Smart System (MSS) — USAREUR-AF

| | |
|---|---|
| **Document** | BSP Evaluator/Coordinator Package |
| **Proponent** | USAREUR-AF C2DAO Training Division |
| **Applies To** | Sprint evaluators, SME support staff, C2DAO event coordinators |

---

## 1. EVALUATOR ROLE

The evaluator is the single point of accountability for Go/No-Go determinations. One evaluator per sprint event. The evaluator:

- Reviews each participant's Project Brief before Day 1 (submitted with enrollment)
- Conducts Day 5 product demo reviews
- Makes the Go/No-Go determination per the standards in BSP_GUIDE.md Chapter 5
...
The evaluator does not provide platform instruction during the sprint. If a participant needs instruction rather than consultation, the evaluator flags this to the SME and, if warranted, documents a No-Go with a TM-30 redirect recommendation.

---

## 2. SME SUPPORT ROLE

SME ratio: 1 SME per ≤8 participants. For larger sprint cohorts (9–16 participants), two SMEs.
...
- Documents any projects that surface TM-40 requirements during the sprint (see Section 3)
...
| 0810–0825 | Sprint structure overview: goals, schedule, standup cadence, evaluation standard |
...
Go when all four standards are met (see BSP_GUIDE.md Chapter 5):
...
**Partial Go** is not a formal evaluation outcome — it is either Go or No-Go. Use the evaluation record notes to document what was completed and what remains for the post-sprint period.
...
- [ ] No-Go participants counseled; remediation plan documented (re-sprint or TM-30 redirect)
- [ ] Any products requiring data steward coordination flagged to the unit Data Steward
- [ ] Sprint after-action report drafted (see Appendix B)
...
```
BUILDER SPRINT — EVALUATION RECORD
Evaluator: [Name, Rank]
Sprint: [Quarter/Year]
...
```
BUILDER SPRINT — AFTER-ACTION REPORT
Sprint: [Quarter/Year]
Coordinator: [Name]
Evaluator: [Name]
...
[Changes to Project Brief criteria, pre-sprint checklist, Day 1 agenda, etc.]
...
*Builder Sprint (BSP) Evaluator/Coordinator Package | Version 1.0 | March 2026*
