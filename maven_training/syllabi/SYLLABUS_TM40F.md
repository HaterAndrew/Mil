...
| | |
|---|---|
| **Level** | TM-40F (Software Engineer Specialist Track) |
...
After this course you can build production-quality software that reads from and writes to the Foundry Ontology via OSDK, manipulates datasets via the Platform SDK, implements computed properties as TypeScript Functions on Objects, writes Action validators with business logic, and delivers Slate applications integrated with the Foundry API.

TM-40F is for developers who need to go beyond the MSS UI. If Workshop and Pipeline Builder can do what you need, TM-30 is sufficient. TM-40F is for building applications, automations, and integrations that the UI tools cannot deliver.

Five days separates the content that could not fit in four: OSDK basics and OSDK advanced are on separate days (Days 1 and 2), eliminating the context collision when they shared a day. Platform SDK, TypeScript Functions on Objects (FOO), and Action validators each receive dedicated lab time. Day 5 covers CI/CD, security, and the evaluated practical exercise.
...
- [ ] Request **OSDK developer access** from C2DAO. This is distinct from standard MSS Builder access. You need a developer token and OSDK access for the specific Object Types used in training. Unit MSS Administrators cannot provision this — contact C2DAO directly.
...
- [ ] Read TM-40F, Chapter 1 (Introduction, role, the 5-layer data stack) — 25 min
- [ ] Read TM-40F, Chapter 9 (Security and Compliance) — read before the labs, not after. Security requirements affect every code pattern.
...
**Evening reading:** TM-40F, Chapter 2 (OSDK Fundamentals) — full chapter; the lab moves fast.
...
**Evening reading:** TM-40F, Chapter 3 (OSDK Advanced) — subscriptions and bulk operations; Chapter 4 (Platform SDK) — transaction patterns and error handling sections.
...
**Evening reading:** TM-40F, Chapter 5 (TypeScript FOO) — testing patterns and N+1 avoidance; Chapter 6 (Actions with Complex Validation) — review validator architecture before Day 4.
...
**Evening reading:** TM-40F, Chapter 7 (Slate) — state management patterns; Chapter 8 (CI/CD and Code Repository Discipline) — read before Day 5.
...
| When | Reading |
|---|---|
| Before Day 1 | TM-40F, Ch 1 (Introduction, 5-layer data stack) |
| Before Day 1 | TM-40F, Ch 9 (Security and Compliance) — read first |
| Day 1 evening | TM-40F, Ch 2 (OSDK Fundamentals) — full chapter |
| Day 2 evening | TM-40F, Ch 3 (OSDK Advanced — subscriptions/bulk) |
| Day 2 evening | TM-40F, Ch 4 (Platform SDK — transactions/error handling) |
| Day 3 evening | TM-40F, Ch 5 (FOO — testing/N+1 avoidance) |
| Day 3 evening | TM-40F, Ch 6 (Actions with Complex Validation) |
| Day 4 evening | TM-40F, Ch 7 (Slate — state management) |
| Day 4 evening | TM-40F, Ch 8 (CI/CD and Repository Discipline) |
...
The validator test suite is evaluated by running the provided test cases against your actual code. Either the validator correctly handles the test case or it does not. All 8 cases must pass.

The Slate application must handle error states. The evaluator will intentionally trigger an error condition — Action execution failure, empty dataset response. If the application crashes or displays an unhelpful error, that task is No-Go. Display a useful, user-visible error message.

The C2DAO deployment checklist asks about credential storage. The correct answer: no credentials in code, no tokens hardcoded, all secrets via environment variables or the Foundry Secrets API. A hardcoded token anywhere in the application fails that checklist item.

---

## Tips From Previous Graduates

- Pagination is tested. If you write an OSDK query that retrieves the first page and stops, the evaluator will use an Object set that requires more than one page. Use `ResourceIterator` and iterate through all results — every time.
- Action execution via OSDK is asynchronous. You get a task ID back, not a result. Poll for completion or use `await` properly. If you treat it as synchronous, your error handling will not catch failed Actions.
- TypeScript FOO performance: avoid per-object API calls in a loop. If you have 500 Objects and your FOO makes 500 API calls, it will time out. Use the bulk query patterns in Chapter 5. The evaluator tests with a 200+ Object set.
- Slate state management is the hardest part. When the user triggers an Action that updates an Object, the table must refresh. Set up a state variable tied to Action completion, not to a timer. Chapter 7 has the pattern.
- The C2DAO deployment checklist asks about credential storage. The answer is always: no credentials in code, no tokens hardcoded, all secrets via environment variables or the Foundry Secrets API. Check every file before submitting.

---

*USAREUR-AF Operational Data Team — UNCLASSIFIED*
*Syllabus TM-40F | Version 2.0 | March 2026*
