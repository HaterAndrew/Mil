...
3. **TRAINING REQUIREMENT.**
   ...
   b. Personnel who build, maintain, or support MSS data products must complete the appropriate higher-level training before performing those duties:
      - TM-20 (Builder): required before building any project, pipeline, Ontology, or Workshop application
      - TM-30 (Advanced Builder): required before serving as a unit data lead or deploying multi-source production data products
      - TM-40 specialist tracks: required before performing specialist functions (ORSA analysis on MSS data, AI workflow authoring, ML model deployment, PM dashboard delivery, KM system management, or application development against the Foundry API)
...
4. **GOVERNANCE STANDARDS.** The following governance standards are in force for all MSS data products. Violations of these standards will result in data steward rejection of the affected product:
   ...
   d. Classification markings will be applied to all datasets and data products at the time of creation. Exports of data will be authorized to destinations commensurate with the classification of the data.
...
5. **TRAINING QUALITY STANDARDS.**
   ...
   c. Sustainment training is required: an annual TM-10 refresher for all personnel, and quarterly build exercises for TM-20 and above personnel. Sustainment requirements are specified in the Mission Training Plan.
...
7. **ACCESS PROVISIONING.** Units will ensure personnel have the appropriate MSS access level provisioned before they attend training. Arriving at training without provisioned access is a unit administrative failure, not a C2DAO failure. Access lead times are published in the Course Administrative Data and the Program of Instruction.

8. **REPORTING.** Units will maintain the Unit Training Status Matrix (see MTP Appendix A) and report MSS training status to C2DAO quarterly. The report will include: total personnel requiring TM-10, number with Go result on file, and any personnel in No-Go remediation status.

9. **EFFECTIVE DATE.** This policy is effective immediately and supersedes any previous informal MSS training guidance.

10. **QUESTIONS.** Questions regarding this policy or the MSS Training Program should be directed to the C2DAO Training POC via the unit MSS Administrator.
